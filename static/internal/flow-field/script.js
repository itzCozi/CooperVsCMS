...
/*
// TODO: Add a variable to limit the amount of times a line can wrap
// around the screen before gradually fading to black
var points = [];
...
var maxWrapCount = 5; // Adjust the maximum wrap count
...
function draw() {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    // Check if the point has wrapped around the screen
    if (points[i].x === 0 || points[i].y === 0 || points[i].x === width - 1 || points[i].y === height - 1) {
      currentWrapCount++;

      // If maxWrapCount is reached, gradually fade to black
      if (currentWrapCount >= maxWrapCount) {
        alpha = map(currentWrapCount, maxWrapCount, maxWrapCount * 2, alpha, 0);
      }
    }
