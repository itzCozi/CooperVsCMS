(function(global, factory) {
  ...
  class ClientV2 extends Client {
    ...
    createBareHeaders(protocol, host, path, port, bareHeaders, forwardHeaders = [], passHeaders = [],
    passStatus = []) {
  ...
  async function fetchManifest(server, signal) {
    const outgoing = await fetch(server, {
      signal
    });
    if (!outgoing.ok) {
  ...
  class BareClient {
    ...
    async fetch(url, init = {}) {
      ...
      for (let i = 0;; i++) {
        ...
        const response = await this.request(method, headers, body, url.protocol, url.hostname, port, url
          .pathname + url.search, cache, signal);
